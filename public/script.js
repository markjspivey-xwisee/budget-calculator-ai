...
async function updateInsights(totalIncome, totalExpenses, remainingBalance) {
    // Show loading state
    document.getElementById('insights').innerHTML = '• Generating AI insights...';
    ...
    try {
        ...
        // Check if insights is available in the response
        if (data.insights) {
            document.getElementById('insights').innerHTML = data.insights;
        } else if (data.error) {
            console.error('Server error:', data.error, data.details);
            document.getElementById('insights').innerHTML = `• Error: ${data.error}. Please try again.`;
        } else {
            console.error('Invalid response format:', data);
            document.getElementById('insights').innerHTML = '• Error: Unexpected response format. Please try again.';
        }
    } catch (error) {
        console.error('Error getting AI insights:', error);
        document.getElementById('insights').innerHTML = `• Error: ${error.message || 'Failed to get insights'}. Please try again later.`;
    }
}
